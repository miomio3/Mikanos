...
EFI_STATUS	SaveMemoryMap(struct MemoryMap *map, EFI_FILE_PROTOCOL *file)
{
	...
	header = "Index, Type, Type(name), PysicalStart, NumberOfPages, Attribute\n";
	len = AsciiStrLen(header);
	file->Write(file, &len, header);
	Print(L"map->buffer = %08lx, map->map_size = %08lx\n", map->buffer, map->map_size);
	for(iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0; iter < (EFI_PHYSICAL_ADDRESS)map->buffer + map->map_size; iter = iter + map->descriptor_size, i++)
	{
		desc = (EFI_MEMORY_DESCRIPTOR *)iter;
		len = AsciiSPrint(buf, sizeof(buf), "%u, %x, %-ls, %08lx, %lx, %lx\n", i, desc->Type, GetMemoryTypeUnicode(desc->Type), desc->Attribute & 0xffffflu);
...
EFI_STATUS	EFIAPI	UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table)
{
	...
	root_dir->Open(root_dir, &kernel_file, L"\\kernel.elf", EFI_FILE_MODE_READ, 0);//kernel_FileOpen
	...
	gBS->AllocatePages(AllocateAddress, EfiLoaderData, (kernel_file_size + 0xfff) / 0x1000, &kernel_sile_size, (VOID *)kernel_base_addr);
